...
```{r}
Bacterialspecies <- read.csv2("Datasets/metaphlan4_species_relab.csv", sep=",",dec=".", row.names=1, header= TRUE )
```


#absolute abundance estimated number of reads species
```{r}
Bacterialspecies <-read.csv2("Datasets/metaphlan4_merged_species_readstats.csv", sep=",",row.names=1, dec=".", header=TRUE)
```

```{r}
metadata <- read.csv("Datasets/metadata_metaphlan.csv", header=TRUE, row.names=1,sep=",", dec =".", stringsAsFactors=FALSE)
```
...
```{r}
dissimilarity_df$Body_site <- factor(dissimilarity_df$Body_site,     # Reorder factor levels
                         c("Lesional", "Non_Lesional", "Top_scalp", "Back_scalp"))
```
```{r}

log_decontam<-ggplot(subset(dissimilarity_df ,  Body_site %in% c("Lesional", "Non_Lesional", "Top_scalp", "Back_scalp")),aes(y = Dissimilarity, x = Body_site, fill=Body_site) ) + 
  geom_boxplot(outlier.shape = NA) + theme_bw() +
  #geom_point(position = position_jitter(width = 0.25), pch=21) +#scale_y_continuous(trans='log10')+ 
  theme(panel.grid = element_blank()) + 
  ylab("Bray-Curtis Dissimilarity") + xlab("") +  scale_fill_manual(values=c("#a6611a", "#dfc27d","#018571", "#80cdc1"))  +  theme(axis.title=element_text(size=18),axis.text.y=element_text(size=14),axis.text.x=element_text(size=14))+guides(fill=FALSE)+ 
  stat_compare_means(label= "p.signif",comparisons = my_comparisons,method="wilcox.test", p.adjust.method="BH", size=4)
...
#Fig.3 a Revisions
```{r}
ggsave("Fig.3a_revisions.png", units="in", width=6, height=7, dpi=400, bg="white")
```
...
# Revised PCoA plot - Revisions 
```{r}


# Define the plot with adjusted label sizes
p_diag <- ggplot(pcoa_meta_new,
                 aes(x = PC1, y = PC2, color = Body_site)) +
          geom_point(aes(shape = Pool, fill = Body_site), size = 3) +
          scale_colour_manual(values = c("#018571", "#80cdc1", "#a6611a", "#dfc27d")) +
...
# Save the plot with the specified dimensions and resolution
ggsave("bray_curtis_PCoA_revisions14May20242.png", plot = p_diag, units = "in", width = 3.54, height = 4.33, dpi = 600, bg = "white")
...
```{r}
data_plot_arranged = data.table::fread("Datasets/maaslin2_18082023.csv", dec=",")
...
```{r}
Bacterialspecies <- read.csv2("Datasets/metaphlan4_genera_relab.csv", sep=",",dec=".", row.names =1, header= TRUE )
```
# Metaphlan4 Kingdom data
```{r}

Bacterialspecies <- read.csv2("Datasets/metaphlan4__kingdom_relab.csv", sep=",",dec=".", row.names =1, header= TRUE )

```
# Metaphlan4 Kingdom data including unclassified fraction
```{r}

Bacterialspecies <- read.csv2("Datasets/metaphlan4_kingdom_unclassified.csv", sep=",",dec=".", row.names =1, header= TRUE )

```
#Metaphlan3 v31 Kingdom data including unclassified data
```{r}
Bacterialspecies <- read.csv2("Datasets/metaphlan3_unclassified_v31_kingdom.csv", sep=",",dec=".", row.names =1, header= TRUE )
```


#Metaphlan3 v30 Kingdom data including unclassified data 
```{r}
Bacterialspecies <- read.csv2("Datasets/metaphlan3_unclassified_v30_kingdom.csv", sep=",",dec=".", row.names =1, header= TRUE )
```
...
```{r}
metadata <- read.csv("Datasets/metadata_metaphlan.csv", header=TRUE, sep=",", dec =";",row.names=1, stringsAsFactors=FALSE)
metadata$Sample <- rownames(metadata)
```


```{r}
Bacterialspecies <- read.csv2("Datasets/metaphlan4_merged_kingdom_readstats.csv", sep=",",dec=".", row.names=1, header= TRUE )
...
```{r}
hostreads_kneaddata <- read.csv("Datasets/Hostreads_kneaddata.csv", header=TRUE, dec=",", sep=",", stringsAsFactors=FALSE)
metadata <- read.csv("Datasets/metadata_metaphlan.csv", header=TRUE, row.names=1,sep=",", dec =".", stringsAsFactors=FALSE)
...
# Protocol
qPCR assays were performed using StepOnePlus real-time PCR system. Reactions were performed in a volume of 20 μl consisting of 10.0 μl of 2x iTaq universal SYBR Green supermix (Bio-Rad Laboratories, Hercules, CA, USA), 2.0 μl DNA template, 0.8 μl (10 µM stock) of each primer, and 6.4 μl nuclease-free water. Amplifications were ran as follows: initial denaturation for 2 min at 95°C, followed by 40 cycles of 15 s denaturation at 95°C and combined anneal/extension 1’ at 60°C

At the end of the qPCR run, a melting curve analysis was performed to confirm product specificity (60-95°C, ΔT per 15 s = 0.3°C). Quantification was performed using a standard curve based on known concentrations of DNA standard dilutions from 107 copies µl-1 down to 10 copies µl-1. All qPCR analyses were conducted in triplicate.
...
# Read data
data.plate1 <- read.csv("Datasets/16S_plate1.csv",header = TRUE, row.names = 1, sep=",", dec=".")
data.plate1 <- data.plate1[,1:19]  # select usefull cols
data.plate2 <- read.csv("Datasets/16S_plate2.csv",header = TRUE, row.names = 1, sep=";")
data.plate2 <- data.plate2[,1:19] # select usefull cols
data.plate3 <- read.csv("Datasets/16S_plate3.csv",header = TRUE, row.names = 1, sep=";")
...
metadata.tibble <- read.csv("Datasets/16S_metadata.csv",header = TRUE, row.names = 1, sep=",")
...
p <- p + theme_bw() + theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) + labs(title="qPCR analysis QSP", subtitle = "qPCR copies as measured in the DNA extract", x="sample ID", y = "Copies/μl")
...
```{r}
# Dilution factor 10
# correction factor 50 µL extract --> *50
...
# non-rarefied data
```{r}
uparse_otus<-read.csv2("Datasets/metaphlan4_merged_species_readstats.csv", sep=",",row.names=1, dec=",", header=TRUE)
```
...
```
```{r}
input_metadata <- read.csv("Datasets/metadata_metaphlan.csv", header=TRUE, row.names=1,sep=",", dec =".", stringsAsFactors=FALSE)
```
...
#save Fig. S8.
```{r}
ggsave("Psoriasiscapitis_humann3.6averages_RByn.png", units="in", width=20, height=12, dpi=400, bg="white")
```

#Fig. 6 and Fig.S9: Humann3.6 mapped
```{r}
input_data <-read.csv2("Datasets/humann36_pathabundance_relab.csv", header = TRUE, sep = ",",  dec = ".", stringsAsFactors = FALSE, row.names=1)
```
...
fit_data = Maaslin2(
    ...
    max_significance = 0.05,
    normalization = "NONE",
    output = "output_pathwaysv36qstat10_NonLesionalvsBackscalp", 
    fixed_effects = c("Body_site"),
    reference = c("Body_site,Back_scalp"))
...
#Complex Heatmap 
```{r}
data_plot_arranged = data.table::fread("Datasets/maaslin2_18082023_humannv36.csv", dec=",")
myplot = data_plot_arranged
...
# Fig. S10
```{r cars}
VF_classes <- read.csv("Datasets/virulence_factors_classes_relative.csv", header=TRUE, dec=".", sep=",", stringsAsFactors=FALSE)
...
#Maaslin2
```{r}
VF_classes <- read.csv("Datasets/virulence_factors_relative_overview.csv", header=TRUE, dec=".", sep=",", stringsAsFactors=FALSE, row.names=1)
#rownames(VF_classes) <- VF_classes$VFcategory
...
```{r}
VF_list <- read.csv("Datasets/virulence_classes.csv", header=TRUE,  sep=",", stringsAsFactors=FALSE, row.names=1)

VF_list$feature <- row.names(VF_list)

VF_list$feature2 <- paste(VF_list$feature, VF_list$VF_Name, sep = " - ")

data_plot_arranged = data.table::fread("Datasets/VF_significantvalues_Maaslin2.csv", dec=",")
...
#AMR elements
```{r cars}
VF_classes <- read.csv("Datasets/AMR_elements_relative_overview.csv", header=TRUE, dec=".", sep=",", stringsAsFactors=FALSE, row.names =1)
...
fit_data = Maaslin2(
    ...
    max_significance = 0.05,
    normalization = "NONE",
    output = "output_AMR_LesionalvsTopscalp", 
    fixed_effects = c("Body_site"),
    reference = c("Body_site,Lesional"))
```
...
fit_data = Maaslin2(
    ...
    max_significance = 0.05,
    normalization = "NONE",
    output = "output_AMR_LesionalvsNon_Lesional", 
    fixed_effects = c("Body_site"),
    random_effects = c("subjectID"),
    reference = c("Body_site,Non_Lesional"))
```
...
##STEP1
```{r}
VF_list <- read.csv("Datasets/AMR_classes.csv", header=TRUE,  sep=",", stringsAsFactors=FALSE, row.names = 1)


VF_list$feature <- row.names(VF_list)
VF_list$feature <- gsub(";", " ", VF_list$feature)

data_plot_arranged = data.table::fread("significantvalues_AMR.csv", dec=",")
...
#save Fig. S13
```{r}
tiff("heatmap_AMR_N36_withnames_orderedlegend.tiff", units='mm', width=350, height=400, res=300)
p
dev.off()
```
...
# Fig. S11
```{r}
mpa_table <- read.csv("Datasets/AMR_elements_absolute_overview.csv", header=TRUE, dec=".", sep=",", stringsAsFactors=FALSE, row.names=1)
mpa_tablet <- as.data.frame(t(mpa_table))
hostreads_kneaddata <- read.csv("Datasets/Hostreads_kneaddata.csv", header=TRUE, sep=",", dec=",", stringsAsFactors=FALSE)
...
hostreads_kneaddata <- read.csv("Datasets/Hostreads_kneaddata.csv", header=TRUE, sep=",", dec=",", stringsAsFactors=FALSE)
mpa_table <- read.csv("Datasets/virulence_factors_absolute_overview.csv", header=TRUE, dec=".", sep=",", stringsAsFactors=FALSE)
...
nmds_res <- metaMDS(bug_mat, distance = "jaccard", k = 2, trymax = 100) #adapt to bray for Bray-Curtis

```

```{r}
nmds_scores <- scores(nmds_res, display = "sites")  # Extracting site scores (samples)
dim(nmds_scores) 
```
...
nmds_scores_df <- as.data.frame(nmds_scores)
colnames(nmds_scores_df) <- c("NMDS1", "NMDS2")

nmds_meta <- bind_cols(nmds_scores_df, metadata)


```
```{r}
nmds_meta_new <- nmds_meta
nmds_meta_new$Body_site <- factor(nmds_meta_new$Body_site, levels = c("Top_scalp", "Back_scalp", "Lesional", "Non_Lesional"))

```
```{r}
#library(ggplot2)
#p_nmds_bray
p_nmds_jacc <- ggplot(nmds_meta_new, aes(x = NMDS1, y = NMDS2, color = Body_site)) +
  geom_point(aes(shape = Pool, fill = Body_site), size = 3) +
  scale_colour_manual(values = c("#018571", "#80cdc1", "#a6611a", "#dfc27d")) +
...
```{r}
figure <- ggarrange(p_nmds_jacc, p_nmds_bray, ncol = 2, nrow = 1,common.legend = TRUE, legend ="right",align="hv", labels=c("A","B"), font.label=list(size=17))
```

save Fig S.5 
```{r}
ggsave("bray_curtis_jaccard_NMDS.png", units="in", width=10, height=5, dpi=400, bg="white")
```
...
```{r}
otutab <- as.matrix(uparse_otus)
#rownames(otutab) <- paste0("OTU", 1:nrow(otutab))
#colnames(otutab) <- paste0("Sample", 1:ncol(otutab))
otutab
colSums(otutab)
min(colSums(otutab))
# Create a taxonomy table with the species names
tax_table <- data.frame(taxonomy = rownames(otutab))
taxmat <- as.matrix(tax_table)
rownames(taxmat) <- rownames(otutab)
colnames(taxmat) <-c("Species")
...
sample_names(physeq)
```
#RAREFACTION CURVES TO ANALYZE
```{r}
#without replacement and without trimming OTUs
oturarefied <- rarefy_even_depth(physeq, sample.size = min(sample_sums(physeq)), rngseed = 711, replace = FALSE, trimOTUs = FALSE)
```

```{r}
otutabrarefied = as.data.frame(otu_table(oturarefied))
```
```{r}

write.csv2(otutabrarefied,"otutabel_rarefied_metaphlan4qstat10.csv")

colSums(otutabrarefied)
```
...
```{r}
divdf_metaphlan <- data.frame(microbiome::alpha(otutabrarefied, index = c("richness_observed")),microbiome::alpha(otutabrarefied, index = c("diversity_shannon")), microbiome::alpha(otutabrarefied, index = c("diversity_gini_simpson")),microbiome::alpha(otutabrarefied, index = c("dominance_gini")))
divdf_metaphlan <- as.data.frame(divdf_metaphlan)

divdf_metaphlan$Sample_name <- rownames(divdf_metaphlan)
```


```{r}
metadata <- read.csv("Datasets/metadata_metaphlan.csv", header=TRUE, dec=".", sep=",", stringsAsFactors=FALSE)
```
```{r}
metadata$Sample_name_full <- metadata$Sample_name
metadata$Sample_name <- metadata$Sample
```

```{r}
metadata_3 <- inner_join(divdf_metaphlan,metadata, by="Sample_name")
```

```{r}
metadata_3_new <- metadata_3                             # Duplicate data
metadata_3_new$Body_site <- factor(metadata_3_new$Body_site,     # Reorder factor levels
                         c("Top_scalp", "Back_scalp", "Lesional", "Non_Lesional"))
...
```
```{r}
a_micr<-ggplot(subset(metadata_3_new ,  Body_site %in% c("Top_scalp", "Back_scalp", "Lesional", "Non_Lesional")),aes(y = observed, x = Body_site, fill=Body_site) ) + 
  geom_boxplot(outlier.shape = NA) + theme_bw() +
  geom_point(position = position_jitter(width = 0.25), pch=21) + #scale_y_continuous(trans='log10')+ 
  theme(panel.grid = element_blank()) + 
  ylab("Richness observed") + xlab("") +  scale_fill_manual(values=c("#018571", "#80cdc1", "#a6611a","#dfc27d"))   +theme(axis.title=element_text(size=21),axis.text.y=element_text(size=14),axis.text.x=element_text(size=20))+guides(fill=FALSE) + stat_compare_means(label= "p.signif",comparisons = my_comparisons,method="wilcox.test", p.adjust.method="BH", size=6)
...
```{r}
b_micr<-ggplot(subset(metadata_3_new ,  Body_site %in% c("Top_scalp", "Back_scalp", "Lesional", "Non_Lesional")),aes(y = diversity_shannon, x = Body_site, fill=Body_site) ) + 
  geom_boxplot(outlier.shape = NA) + theme_bw() +
  geom_point(position = position_jitter(width = 0.25), pch=21) + #scale_y_continuous(trans='log10')+ 
  theme(panel.grid = element_blank()) + 
  ylab("Shannon") + xlab("") +  scale_fill_manual(values=c("#018571", "#80cdc1", "#a6611a","#dfc27d"))  +  theme(axis.title=element_text(size=21),axis.text.y=element_text(size=14),axis.text.x=element_text(size=20))+guides(fill=FALSE) + stat_compare_means(label= "p.signif",comparisons = my_comparisons,method="wilcox.test", p.adjust.method="BH", size=6)
...
```{r}
c_micr<-ggplot(subset(metadata_3_new ,  Body_site %in% c("Top_scalp", "Back_scalp", "Lesional", "Non_Lesional")),aes(y = diversity_gini_simpson, x = Body_site, fill=Body_site) ) + 
  geom_boxplot(outlier.shape = NA) + theme_bw() +
  geom_point(position = position_jitter(width = 0.25), pch=21) + #scale_y_continuous(trans='log10')+ 
  theme(panel.grid = element_blank()) + 
  ylab("Simpson") + xlab("") +  scale_fill_manual(values=c("#018571", "#80cdc1", "#a6611a","#dfc27d"))  +  theme(axis.title=element_text(size=21),axis.text.y=element_text(size=14),axis.text.x=element_text(size=20))+guides(fill=FALSE)+ stat_compare_means(label= "p.signif",comparisons = my_comparisons,method="wilcox.test", p.adjust.method="BH", size=6)
...
```{r}
d_micr<-ggplot(subset(metadata_3_new ,  Body_site %in% c("Top_scalp", "Back_scalp", "Lesional", "Non_Lesional")),aes(y = dominance_gini, x = Body_site, fill=Body_site) ) + 
  geom_boxplot(outlier.shape = NA) + theme_bw() +
  geom_point(position = position_jitter(width = 0.25), pch=21) + #scale_y_continuous(trans='log10')+ 
  theme(panel.grid = element_blank()) + 
  ylab("Dominance Gini") + xlab("") +  scale_fill_manual(values=c("#018571", "#80cdc1", "#a6611a","#dfc27d"))  +  theme(axis.title=element_text(size=21),axis.text.y=element_text(size=14),axis.text.x=element_text(size=20))+guides(fill=FALSE) + stat_compare_means(label= "p.signif",comparisons = my_comparisons,method="wilcox.test", p.adjust.method="BH", size=6)
```
```{r}
figure_rare <- ggarrange(a_micr,b_micr, c_micr, d_micr, ncol=2, nrow=2, align="hv", labels=c("A","B", "C", "D"), font.label=list(size=17))
```
```{r}
ggsave("alphadiverisity_rarefied_nonreplacenontrim.png", units="in", width=16, height=10, dpi=400, bg="white")
```

#qPCR ACTB and 18S rRNA gene 

```{r}
data_pso_all_16S <- read.csv("Datasets/qPCR_16S_ACTB_normalized.csv", header=TRUE, dec=".", sep=",", stringsAsFactors=FALSE)
```
```{r}
data_pso_all_18S <- read.csv("Datasets/qPCR_18S_ACTB_normalized.csv", header=TRUE, dec=".", sep=",", stringsAsFactors=FALSE)
```

```{r}
# Assuming your dataframe is named data_pso_all_16S
subset_sebaceous <- data_pso_all_16S[data_pso_all_16S$site.type == "sebaceous", ]
...
# Your ggplot code
qPCR_result <- ggplot(subset(subset_sebaceous, type %in% c("Lesional", "Non_Lesional")), aes(y = bacterial.norm, x = type, fill = type)) +  
  scale_y_continuous(trans = 'log10') +
  geom_boxplot(outlier.shape = NA) +  
  theme_bw() +  
  geom_point(position = position_jitter(width = 0.25), pch = 21, size=3) +
  theme(panel.grid = element_blank(), axis.text.x = element_text(size = 20), axis.text.y = element_text(size = 14), axis.title = element_text(size = 21)) +   
  ylab(expression(Log[10] ~ "(16S rRNA Copies/ACTB Copies)")) + xlab("") + scale_fill_manual(values = c("#a6611a", "#dfc27d"))  +guides(fill=FALSE)+ 
  stat_compare_means(label= "p.signif",comparisons = list(c("Lesional", "Non_Lesional")),method="wilcox.test", p.adjust.method="BH", tip.length=0.01, textsize=8,size=6, bracket.size = 0.3, vjust=0)
...
```{r}
subset_sebaceous_18s <- data_pso_all_18S[data_pso_all_18S$site.type == "sebaceous", ]
```

```{r}

# Your ggplot code
qPCR_result_18S <- ggplot(subset(subset_sebaceous_18s, type %in% c("Lesional", "Non_Lesional")), aes(y = fungal.norm, x = type, fill = type)) +  
  scale_y_continuous(trans = 'log10') +
  geom_boxplot(outlier.shape = NA) +  
  theme_bw() +  
  geom_point(position = position_jitter(width = 0.25), pch = 21, size=3) +
  theme(panel.grid = element_blank(), axis.text.x = element_text(size = 20), axis.text.y = element_text(size = 14), axis.title = element_text(size = 21)) +   
  ylab(expression(Log[10] ~ "(18S rRNA Copies/ACTB Copies)")) + xlab("") + scale_fill_manual(values = c("#a6611a", "#dfc27d"))  +guides(fill=FALSE)+ 
  stat_compare_means(label= "p.signif",comparisons = list(c("Lesional", "Non_Lesional")),method="wilcox.test", p.adjust.method="BH", tip.length=0.01, textsize=8,size=6, bracket.size = 0.3, vjust=0)
...
```{r}
figure_count <- ggarrange(log_decontam, qPCR_result, qPCR_result_18S, ncol=3, nrow=1, align="hv", labels=c("a","b", "c"), widths = c(2,1,1), font.label=list(size=17))
```
```{r}
ggsave("Fig.2_revisions.png", units="in", width=20, height=8, dpi=300, bg="white")
```